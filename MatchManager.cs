...
public class MatchManager : MonoBehaviourPunCallbacks, IOnEventCallback
{
    ...
    void Update()
    {
        if (Input.GetKey(KeyCode.Tab) && state != GameState.Ending)
        {
            ...
            ShowLeaderboard();
        } else if (state != GameState.Ending) { UIController.instance.leaderboard.SetActive(false);  }
    }
    public void OnEvent(EventData photonEvent)
    {
        if (photonEvent.Code < 200) //first 200 is the default number of events we can handle manually before f-ing up the photons internal event handling
        {
    ...
    {
        object[] package = new object[4]; //4 slots because: name actors kills deaths => index 2 and 3 are zero upon joining since they didnt do anything yet
        package[0] = username;
